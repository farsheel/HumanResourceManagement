...
<div id="panel-option-values" class="panel panel-default">
    ...
    <table class="table table-bordered table-striped margin-b-none">
        ...
        <tbody class="form-options-body">
            <?php foreach ($modelDocuments as $index => $modelDocuments): ?>
                <tr class="form-options-item">
                    <td class="sortable-handle text-center vcenter" style="cursor: move;">
                        <i class="fa fa-arrows"></i>
                    </td>
                    <td class="vcenter">
                        <?= $form->field($modelDocuments, "[{$index}]vchr_document_title")->label(false)->textInput(['maxlength' => 128]); ?>
                    </td>
                    <td>
                        <?php if (!$modelDocuments->isNewRecord): ?>
                            <?= Html::activeHiddenInput($modelDocuments, "[{$index}]id"); ?>
                            <?= Html::activeHiddenInput($modelDocuments, "[{$index}]image_id"); ?>
                            <?= Html::activeHiddenInput($modelDocuments, "[{$index}]deleteImg"); ?>
                        <?php endif; ?>
                         <?php
                           /* $modelImage = Image::findOne(['id' => $modelDocuments->image_id]);
                            $initialPreview = [];
                            if ($modelImage) {
                        ...
                        ?>
                        <?= $form->field($modelDocuments, "[{$index}]file")->label(false)->widget(FileInput::classname(), [
                            'options' => [
                                'multiple' => false,
                                'accept' => 'image/*',
